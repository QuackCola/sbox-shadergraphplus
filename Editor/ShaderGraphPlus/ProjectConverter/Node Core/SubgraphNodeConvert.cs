...
internal class SubgraphNodeConvert : BaseNodeConvert
{
	...
	public override IEnumerable<BaseNodePlus> Convert( ProjectConverter converter, ShaderGraphBaseNode oldNode )
	{
		...
		if ( !projectConverter.Errored )
		{
			...
			if ( asset == null )
			{
				SGPLogger.Error( $"Unable to register asset at path \"{fullPath}\"" );
			}
			else
			{
				SGPLogger.Info( $"Registerd subgraphplus asset at path \"{fullPath}\"" );
			}
		...
		{
			SGPLogger.Error( $"Unable to convert shadergraph function at path \"{fullPath}\"" );
